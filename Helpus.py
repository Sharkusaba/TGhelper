...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    keyboard = [
        [create_button("ℹ️ О нас", callback_data="about")],
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "main_menu":
        ...
        for region in REGIONS.keys():
            keyboard.append([create_button(f"📍 {region}", callback_data=f"region_{region}")])
        keyboard.append([create_button("🔙 Назад", callback_data="main_menu")])
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_caption(
            caption="🗺️ Выберите город:",
            reply_markup=reply_markup
        )
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_caption(
            caption="✍️ Для вступления в наши ряды заполните анкету по ссылке ниже:",
            reply_markup=reply_markup
        )
